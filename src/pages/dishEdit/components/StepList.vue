<template>
  <q-list bordered separator>
    <q-item-label header class="flex flex-center">{{ stepDisplayName }}</q-item-label>
    <q-separator/>
    <q-scroll-area
      ...
      style="height: 300px;"
    >
      <q-slide-item v-for="step in steps" :key="step.name" @left="onLeft" @right="onRight">
        <template>
          <q-icon name="delete"/>
        </template>
        <q-item>
          <q-item-section>
            {{ `${secondsToMMSS(step.time)}，${step.name}(${step.shape})，${step.weight}克，槽位${step.slot} ` }}
          </q-item-section>
        </q-item>
      </q-slide-item>
    </q-scroll-area>
    ...
    <q-item-label header class="flex flex-center q-py-sm">
      <q-btn dense icon="add" class="" round @click="ingredientDialog.shown = true"/>
    </q-item-label>
