  ...
  >
    <div class="row q-mb-md" v-for="(r) in rows" :key="'r' + r">
      <div class="col q-mr-md" v-for="col in 4" :key="col">
        <q-card v-if="last_row_cols===0 || (last_row_cols!==0&&r!==rows) || (r === rows && col <= last_row_cols)"
                class="img-item" @click="onImgClick(dishes[(r - 1) * 4 + col - 1].id)">
          <q-img
            ...
            spinner-color="white"
          >
            <div class="text-subtitle2 absolute-bottom">{{ dishes[(r - 1) * 4 + col - 1].name }}</div>
          </q-img>
        </q-card>
      </div>
    </div>
