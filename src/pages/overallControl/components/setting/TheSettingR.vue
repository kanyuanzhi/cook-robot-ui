    ...
    <div class="col-9 flex">
      ...
      <q-item-section>
        <div class="row items-center q-col-gutter-md">
          <div class="col-6">
            <OptionalInput v-model="speed" label="旋转速度" :min="0" :max="200" :step="1"/>
          </div>
          <div class="col-6">
            <OptionalInput v-model="circles" label="正反转圈数" :min="0" :max="10" :step="1"/>
          </div>
        </div>
      </q-item-section>
    </div>
...
const onBtnClick = (action) => {
  const instruction = createPLCInstruction(props.type, 0, action, [speed.value, circles.value]);
